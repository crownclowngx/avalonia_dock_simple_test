...
> 评审日期：2026-08-21。
>
> 事实基线：当前 `dev-重构-2026年8月18日` 分支源码、Managed Plugin v1 文档与本轮 Release 门禁。
>
> 重要说明：本文中的 V2 类型、包、文件格式和 G0–G14 均是目标设计，不得引用为当前能力。
...
- **变更**：增加 V2 删除面清单、目标依赖白名单和当前消费者矩阵；不改生产行为。
- **插件影响**：无。
- **验证**：Release 零警告构建、三套 Host 测试、四插件测试、SDK/API/包门禁和 Windows Smoke。

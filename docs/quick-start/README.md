...
Host Dock Adapter。当前仓库处于未发布 V3 G3：版本为 3.0.0，Document 使用互斥 New/Restore 激活，
持久化 Document 使用修订快照和指定修订确认，其他 API 与运行语义仍沿用 V2 G14；
教程不使用 Legacy、Strategy、独立 View 注册或 Dock 类型。

## 完成后你将得到什么

- 一个由严格 `plugin.manifest.json` 精确加载的 V3 G3 插件；

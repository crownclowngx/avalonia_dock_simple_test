...
> 当前仓库处于 V2 G1：Plugin SDK 与四插件版本为 `2.0.0`，G3 前的 Host/Common 字段都投影
> `[2.0.0, 3.0.0)`。最终 SDK 与 manifest v2 尚未完成，因此本教程当前只用于仓库内阶段联调；
> 外部作者应继续以 `managed-plugin-v1.0.0` 复核历史契约，不得发布或承诺 V2 兼容。

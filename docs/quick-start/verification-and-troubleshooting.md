...
- [ ] 清单版本与入口 `AssemblyVersion` 一致，模块 `PluginId` 与清单一致；
...
- [ ] View 能按 `*.ViewModels.*ViewModel` → `*.Views.*View` 约定找到。
...
2. **三个身份必须一致。** 清单 `pluginId`、模块 `PluginId` 和扩展 ID 命名空间应指向同一插件所有者。
3. **一个程序集只能有一个模块。** 模块需要 public 无参构造；Managed 策略可以使用构造注入。

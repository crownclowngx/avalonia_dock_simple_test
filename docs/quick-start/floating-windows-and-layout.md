...
| 关闭浮窗最后一个 Document 标签 | 先确认与排空，获准后连同空浮窗一起回收；取消则保留原窗口和内容 |
| 同浮窗仍有其他 Document 或 Tool | 只关闭目标文档并清理空组，保留其余内容和窗口 |
| 关闭主窗口最后一个文档标签 | 保留主窗口和文档区，仍可继续新建 |
...
可通过 **文件 → 重新启动 Host…** 自动完成关闭与再次启动。它沿用下述布局保存和恢复规则，保存失败或取消时保留原工作区；操作步骤见[重启指南](restart-host.md)。

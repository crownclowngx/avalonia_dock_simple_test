...
> V3 G6 当前事实：源码处于未发布的产品/SDK/四插件 `3.0.0` 版本线；public API 仍为 Unshipped，
> G2–G6 已分别完成修订保存、互斥激活、注册所有权、插件私有消息和 Workspace/Dock 拆分；
...
| [Managed Plugin V3 破坏式重构任务书](./design/host-v3-breaking-refactor-plan.md) | V3 保存修订、激活语义、注册所有权、消息边界和 Workspace 解耦的 G0–G14 计划 | 实施中；G0–G6 已完成，G7–G14 尚未实施 |
...
- [Managed Plugin V3 破坏式架构重构任务书](./design/host-v3-breaking-refactor-plan.md)：
  ...
  分离为目标的 G0–G14 计划；当前完成 G0–G6，Document 保存使用 V3 G2，激活使用 V3 G3，注册所有权使用 V3 G4，插件私有消息使用 V3 G5，Workspace/Dock 使用 V3 G6，其他生产语义仍由 V2 G14 签署。

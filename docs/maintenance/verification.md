...
[V23 本地 ZIP 安装契约](../reference/plugin-installation.md)已实现，使用[专用回归矩阵](host-v23-local-zip-installation-verification.md)验证包协议、版本决策、事务中断、跨进程租约、启动恢复及看板。实际测试与完整 verify 以[开发记录](../archive/records/host-v23/development-acceptance.md)及其 JSON 为准；现有 PackageAcceptance 恰好一项的规则保持，不依赖外部插件仓库，不使用 AIFLOW、Windows CI 或发布门禁。

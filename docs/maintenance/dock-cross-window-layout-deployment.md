...
> 用途：把已验证的 Document 跨窗修复部署到本机 `D:\data\avalonia`。日期：2026-09-17。用户已授权提交、编译和部署；实际完成状态、源码提交、安装版摘要及备份位置以[本次部署证据](../archive/records/dock-area-fill/cross-window-layout-deployment-20260917.json)为准。

## 输入与交付范围

源码修复为 `19c0b3b`，完整开发验证记录在[修复证据](../archive/records/dock-area-fill/cross-window-layout-fix-evidence.json)：1059 项本地 verify、68 项门禁工具自测和七项独立框架测试通过。后续部署提交仅同步文档，不改变已经验证的生产代码。

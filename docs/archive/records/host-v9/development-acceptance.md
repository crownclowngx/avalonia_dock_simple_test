...
> 日期：2026-09-16。方案：[Host V9](../../plans/host-v9-avalonia-dock-upgrade-plan.md)。
...
最终 verify 汇总（历史产物位置，文件已清理：`artifacts/gate/20260916-000949-058b0f22ad98/summary.json`）记录源码工作区 clean、Release 构建 0 警告/错误，restore、build、tests、contracts、packages、package-acceptance 六阶段全部通过。
...
覆盖率摘要（历史产物位置，文件已清理：`artifacts/host-v9/coverage/summary.json`）、HTML 报告（历史产物位置，文件已清理：`artifacts/host-v9/coverage/merged/index.html`）和[提交内证据摘要](development-evidence.json)保存原始报告位置与 SHA-256。复现口径：

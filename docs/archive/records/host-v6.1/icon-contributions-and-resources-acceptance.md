...
对应 [设计方案](../../plans/host-v6.1-extensible-icon-contributions-plan.md) 与 [开发说明](../../../quick-start/plugin-icons.md)。
...
最新 Gate 结果（历史产物位置，文件已清理：`artifacts/gate/20260911-023642-cc41a2ef0755/summary.json`） 为全部通过：
...
合并报告（历史产物位置，文件已清理：`artifacts/v6.1/coverage-final/merged/Cobertura.xml`）：行 **87.38%**，分支 **71.27%**；
对应门槛 **84.39% / 70.58%**，均通过。SDK 和 Icons 行数没有混入 Host 指标。

图像验收文件位于 screenshots（历史产物位置，文件已清理：`artifacts/v6.1/screenshots`）：旧目录、新树与功能中心的浅/深色，
...
所有生成项目位于仓库外的任务临时目录，路径记录在 consumer-root.txt（历史产物位置，文件已清理：`artifacts/v6.1/consumer-root.txt`）。
...
2. 生成 DemoPlugin，编译 Plugin/Standalone/Tests，4 个模板测试通过；真实 Standalone App + MainView 的
   Headless Skia 探针验证路径、画布、主题画刷和图像，见 预览日志（历史产物位置，文件已清理：`artifacts/v6.1/preview-probe.log`）。
...
4. 通过真实部署 Target 验证 Host DLL、UI SDK DLL、伪扩展 `MyAvaloniaManagement.Icons.Other.dll`
   及子目录中的 Core SDK DLL 均按共享库规则拒绝，见 四个负例（历史产物位置，文件已清理：`artifacts/v6.1/forbidden-assets-final.log`）。
...
8. 最终使用 NuGet.org 公开源安装 `MyAvaloniaManagement.Plugin.Templates@1.4.1`，在独立 hive 中生成
   ...
   完整日志（历史产物位置，文件已清理：`artifacts/v6.1/public-template-final.log`） 与
   公开安装诊断日志（历史产物位置，文件已清理：`artifacts/v6.1/public-template-install-diagnostic.log`） 可复验整个过程。
   ZIP 的 五项文件清单（历史产物位置，文件已清理：`artifacts/v6.1/public-template-zip-entries.txt`） 确认私有 Icons DLL 存在、
...
日志见 基础包（历史产物位置，文件已清理：`artifacts/v6.1/publish-foundation.log`）、模板 1.4.0（历史产物位置，文件已清理：`artifacts/v6.1/publish-template.log`）
与 模板 1.4.1（历史产物位置，文件已清理：`artifacts/v6.1/publish-template-patch.log`）。
...
公开载荷核对（历史产物位置，文件已清理：`artifacts/v6.1/public-payload-checks.json`） 保存两种整体哈希与内容核对结果。

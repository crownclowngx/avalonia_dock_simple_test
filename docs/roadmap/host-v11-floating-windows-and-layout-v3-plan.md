...
后续补丁单独跟踪：[V11-P1 Tool 上下分割](host-v11-p1-tool-split-fix-plan.md) 与 [V11-P2 Tool 浮窗关闭黑框](host-v11-p2-tool-window-close-fix-plan.md)。P2 处理按钮重复命令、最后工具窗口回收、取消及位置保留，不改变本计划的实例所有权、V3 协议或文档恢复范围。历史安装目录交付与各补丁源码验证分别记录。
...
实施调整：日常保存后台执行；最终队列不捕获 Dispatcher，可在无在途操作时同步排空，保留既有原生一次关闭语义。需要异步确认或命令排空时取消首次关闭，准备完成再重试；不阻塞等待需要 Dispatcher 的工作。写入错误报告为未保存本次布局，保留上一有效文件并恢复窗口操作，不改变文档保存判断。保存任务、事件订阅和窗口对象的清理由实际创建者配对执行；Runtime 失败回滚不创建窗口或临时解析存储服务。
...
- [x] 对锁定 Dock 版本核实 Float/FloatAll、窗口根登记、原生关闭回调、拖动事件和 InitLayout 时序，以最小 Host 测试证明，不只引用最新版在线示例。
...
- [x] 实现 DTO、严格 JSON、纯校验、V2 只读转换和过滤文档后的树归并。
- [x] 建立正常、坏格式、旧格式、隐藏浮窗和缺失工具夹具及往返测试。
...
- [x] 开放内容及合法组浮动，创建 Host 浮窗适配与窗口登记。
- [x] 完成范围关闭、整组确认、目标命令排空、取消恢复和一次性关闭许可。
- [x] 接入主窗口退出与现有 Runtime 关闭链，保持幂等释放和失败保留语义。
- [x] 补 View 迁移、事件退订、缓存与 Scope 释放测试。

通过条件：浮动/回停复用原实例；取消不拆组；关闭子窗不结束 Runtime；关闭全部窗口无残余订阅与错误释放。

### G4：跨窗口命令与原生交互

- [x] 复用窗口快捷键、命令面板、Owner 选择与全屏租约逻辑。
- [x] 工具中心和页面列表激活实际承载窗口，最小化时先恢复。
- [x] 保留指针/正文保护并通过原生 HostWindow Headless、快捷键与 View 回归。
- [ ] 真实桌面验证指针捕获、Esc、失活、鼠标拖出再拖回及窗口主题。

通过条件：浮窗命令目标正确、一次触发一次执行、窗口关闭后的迟到通知不重新绑定失效对象。

### G5：持久化、自动保存与恢复事务

- [x] 接入 V3 Store、备份、独占写入、只读实例和未来 schema 保护。
- [x] 接入保存调度、UI 快照、串行写入和退出前最终提交。
- [x] 接入可用项投影恢复、保留项合并、非空浮窗显示与失败回滚。
- [x] 更新当前 Layout 版本属性、VersionPolicyTests 与实际受影响的开发契约引用。

通过条件：重启工具布局稳定，文档不重开，旧数据保留，文件故障和实例竞争不会覆盖有效布局。

### G6：屏幕适配与用户恢复入口

- [x] 实现并测试位置计算、DPI 换算、正常尺寸/最大化状态。
- [x] 接入找回窗口与重置布局命令，保留文档实例和修改状态。

...
- [x] 阅读本计划及链接的当前契约，记录 `git status`、源码 revision 和实际依赖版本。
- [x] 根据用户截图记录故障并检查两条入口源码；原生遮挡复现归入下方未执行矩阵。
- [x] 核对固定上游提交、补丁入口及包还原流程；建立补丁维护目录与构建入口。
- [x] 将“区域空白处不能 Fill”转成框架侧失败测试，同时保留方向按钮对照用例。

退出条件：复现明确，测试失败原因对应真实问题，能够生成默认策略关闭的基线包。

### P1：实现判定和预览

- [x] 新增默认关闭的区域策略及共用判定结果，区分未命中、有效命中和明确拒绝。
- [x] 实现合法内容区域 Fill、方向优先和标签栏语义保留。
- [x] 连接两条拖放入口，补齐最新坐标提交、失效目标拒绝和预览清理。
- [x] 验证局部、全局及 Tool 全宽策略；补丁和 Host 各层自动化分别执行，桌面像素效果待补。

退出条件：框架侧规则用例通过，开启 / 关闭策略可对照验证。

### P2：接入 Host 与回归

- [x] 完成补丁包版本、仓库源和锁文件接入；独立缓存重建结果见开发证据。
- [x] 宿主样式启用策略，自动化覆盖主窗口、纯文档浮窗与工具浮窗。
- [ ] 混合分组与原生窗口手势完成真实桌面验收。
- [x] 验证 Host 原模型 / View / Scope、活动项、工具 Pin 状态、空窗清理及布局保存协议。
...
- [x] 更新使用指南、Host 设计说明、专项验证指南和本计划状态。
- [x] 接入完整本地主仓 Gate verify 的补丁前置阶段；本轮运行结果独立记录，不预填通过。
- [x] 下方桌面矩阵如实标为未执行；当前会话不提供原生桌面输入能力。
- [x] 建立非嵌入 JSON 证据与待办链接；最终源码、包 / DLL 身份及实际命令结果在运行后写入。
...
Host 新增 `DockAreaFillUiTests` 验证可观察结果，并包含真实标签鼠标输入闭环。直接调用 DockManager 移动成功不能证明原生遮挡正确；原生桌面结果仍单独补验。

### 7.2 命令顺序

在主仓根目录执行。第一条脚本已交付，负责固定源码上的补丁测试、可复现本地打包与成功收据校验。
...
| 编号 | 操作 | 通过标准 | 状态 |
| --- | --- | --- | --- |
| M01 | Document 浮窗拖到空主文档区，避开所有按钮松开 | 整区预览，Document 回停，原实例保留 | 未执行：无原生桌面输入能力 |
| M02 | 目标已有多个标签，拖到内容区松开 | 合并到该组，已有内容和标签顺序符合框架约定 | 未执行：无原生桌面输入能力 |
| M03 | 目标已左右 / 上下分组，在各组内容区分别投放 | 仅命中的组接收，其他组保持原布局 | 未执行：无原生桌面输入能力 |
| M04 | 分别命中四个局部方向按钮及四个全局方向按钮 | 对应方向有效，区域 Fill 不抢占，预览匹配最终范围 | 未执行：无原生桌面输入能力 |
| M05 | Tool → Tool、浮窗内部、主文档区分别做上下分割 | 前两者局部；主文档区保留全宽；分隔条可继续拖动 | 未执行：无原生桌面输入能力 |
| M06 | 用被拖动的浮窗盖住目标中央按钮，在合法区域松开 | 无需露出或命中中央按钮即可回停，提示清晰 | 未执行：无原生桌面输入能力 |
| M07 | 源浮窗同时遮住鼠标下方的目标内容，连续拖动 | 能识别其背后的合法目标，不把源浮窗或提示层当成目标 | 未执行：无原生桌面输入能力 |
| M08 | 在内容区与方向按钮间往返，再离开目标松开 | 预览及时切换和消失，区域外保持浮动 | 未执行：无原生桌面输入能力 |
| M09 | 同组排序、跨组标签投放、整组浮窗回停 | 标签行为正常，组内内容无丢失和重复 | 未执行：无原生桌面输入能力 |
| M10 | 制造不允许 Fill / 分割的目标，及目标失效场景 | 无误导预览，不发生拒绝分割后偷偷合并 | 未执行：无原生桌面输入能力 |
| M11 | 全屏限制、关闭取消、源 / 目标窗口关闭及连续多次拖动 | 没有空壳、残留提示、失效引用或提前释放 | 未执行：无原生桌面输入能力 |
| M12 | 100% / 150% / 200% 与不同缩放双屏、负坐标屏幕 | 指针、按钮和预览对齐，跨屏命中正确 | 未执行：无原生桌面输入能力 |
| M13 | 回停后隐藏 / 显示工具，保存并重启 | 工具分组与比例恢复；Document 按既有契约不自动重开 | 未执行：无原生桌面输入能力 |
| M14 | 关闭区域策略后重复核心场景 | 恢复原精确按钮行为，原方向停靠仍可用 | 未执行：无原生桌面输入能力 |

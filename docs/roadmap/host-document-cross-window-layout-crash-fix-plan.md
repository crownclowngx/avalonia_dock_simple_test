...
上述 B 的交付设计已接入：固定来源、构建锁、独立七项验证、PublicSign、APICompat、两个独立路径/包目录的 DLL/PDB 字节对照，以及 build/test/publish/bundle 的资产校验。具体结果随非嵌入证据记录，不以本方案代替产物身份。

## 7. 实施顺序与各阶段产出

### P0：固化宿主失败

- [x] 核对工作区、安装版与依赖版本；保留原部署记录和 Windows 异常。
- [x] 将诊断样例迁为正式 Host 回归，补 T01/T02/T05/T15 的关键组合。
- [x] 用测试夹具记录旧根、目标根、实际父级与模板调用时序，不记录用户文档内容。
- [x] 保存失败 TRX，确认是相同布局异常，而非编译错误、插件缺失或错误测试坐标。

产出：可稳定重现的 Host 红灯、源/目标根变化记录，以及是否满足 A 的安全交接边界的结论。

### P1：实现最小修复并锁定路线

- [x] 按第 4 节在复用边界实现 A；优先私有方法，不改变 SDK、Dock 区域命中或布局 schema。
- [x] 运行旧窗先布局、新窗先布局、重入和延迟模板的关键回归。
- [x] 若满足切换条件，停止扩大 A，依第 6 节完成 B 的技术与交付设计，更新本节的实际选择和理由。
- [x] 不保留互相掩盖的两套修复；每个生产改动都要能对应到失败原因或回归断言。

产出：关键红灯转绿、中文设计注释、实际修改清单；尚未宣称全面验收通过。

### P2：补齐回归与本地开发验证

- [x] 完成 T01–T15，测试用例命名及注释使用中文；对照条件使用固定输入和确定性时序。
- [x] 回归现有正文回收、浮窗关闭、指针捕获、工具分屏与 Layout V3。
- [x] 先定稿当前 Markdown，再执行完整本地 verify。
- [x] 如修改构建门禁或依赖脚本，增加失败传播、缺失 DLL、摘要不一致及错误缓存回归。
- [x] 保存新一次的通过、失败、跳过数量及源码/产物身份，不沿用此前 1039 项结果作为修复证明。

当前实现的执行命令如下；每次结果由当次 TRX 和非嵌入证据确认：
...
- [x] 保留修复前同一异常的确定性 Host 红灯。
- [x] 选择的方案对两种窗口布局顺序均通过，并覆盖重入和延迟模板。
- [x] T01–T15 已落实到新专项和既有回归；最终全量执行状态见本轮证据，生命周期和布局状态断言保留。
- [x] 已接入本地 verify、构建来源与 DLL 身份检查；本轮最终运行结论由非嵌入证据记录。
- [ ] M01–M03 原故障场景实际通过；其余桌面限制单列，不以测试总数掩盖。
- [x] 当前文档、构建来源、红灯证据与最终证据入口已同步。

## 9. 后续部署和回退

本轮已收到实施与 Git 提交授权，执行源码修复、本地测试和隔离产物核对。尚未收到本次安装部署指令，因此 `D:\data\avalonia` 保持原安装版；不把此前一次部署授权写成永久自动发布任务。

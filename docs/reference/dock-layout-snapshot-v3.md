...
> 用途：V11 工具布局的唯一线格式与恢复契约。状态：当前，已接入生产恢复、自动保存和退出流程；Host 人工验收已[确认通过](../archive/records/host/manual-acceptance-20260921.md)。核对日期：2026-09-21。实施结果见 [V11 记录](../archive/records/host-v11/development-acceptance.md)。
...
仅支持 Layout schema 3。`layout-v1.json` 和 `layout-v2.json` 不探测、不读取、不迁移、不修改或隔离；只有旧文件时建立默认全隐藏工具布局，后续仅保存 V3。旧文件清理不属于运行时职责。已退役内置工具仍由当前布局合并精确排除，暂时缺失插件的恢复记录继续保留。

V3 主文件优先，其次有效 V3 备份；都不可用时采用默认布局。未来 schema 使用独立错误 `LAYOUT_SCHEMA_FUTURE`，宿主保留文件并采用只读默认布局，不以较旧备份降级。当前文件 I/O 状态不确定时本会话保持只读；损坏文件继续保留原件与诊断副本。

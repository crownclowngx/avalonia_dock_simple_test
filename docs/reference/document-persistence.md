...
Host 不读取、迁移或写回 Document V1；旧设计仅供历史追溯。插件内容 schema 与 Host 信封 schema 独立，插件必须先完整解码并验证内容，再一次提交模型状态。

`CaptureSaveSnapshotAsync` 不是保存提交点；它返回同一稳定观察区间内的
...
单页、浮窗范围与应用退出共用确认/保存和命令排空后的最终脏状态检查。原本干净的文档在命令取消
回调或排空等待期间产生修改，也必须撤销许可并重新开放命令。`AcceptChanges` 异常后的结果仍是
“已保存但有警告”，若文档仍脏则保留页面；只有备份写入失败、文档已干净时不阻止关闭。明确选择
放弃仅授权确认时的脏文档，不能扩大到范围中后来变脏的其他文档。旧确认转发入口已删除。
...
重点覆盖 New/Restore 互斥、严格六字段、长度/深度预算、并发同路径激活、坏主文件恢复、旧修订确认、保存后仍脏、取消不释放和最终关闭幂等。覆盖率及 seal 前提见[主仓验证](../maintenance/verification.md)，不沿用已退役阶段脚本的门槛或通过数字。

当前规则不允许为回滚恢复 V1 双 reader，也不自动迁移、删除或降级用户文件。历史方案和阶段结果见[归档](../archive/README.md)。

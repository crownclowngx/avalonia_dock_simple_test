...
起点和终点都在强制 GC/LOH 压缩后等待至少 15 秒，并要求线程数与句柄数连续稳定，
以排除 ThreadPool 原生回调线程仍在退坡的瞬时状态。
基线前还会按逻辑处理器数预热并记录 ThreadPool 工作者数量；这是为了让全局工作池
基础设施同时存在于起点和终点，不会从结果中豁免 Semaphore、Event 或 Thread 句柄。

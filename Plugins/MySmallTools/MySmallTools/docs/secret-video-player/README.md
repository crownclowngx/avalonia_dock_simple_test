...
正式发布包只包含 `Controls/SmallTools/`，可直接解压到宿主根目录。单一发布入口会串行完成 Release 零警告构建、132 项插件测试、20 项宿主扫描测试、Manifest/ZIP 校验、64/512 MiB 内存门禁和两轮真实窗口播放门禁：
...
- 文件夹视频库只扫描当前层的 `.secvid`，不提供递归、目录监听或密码持久化；连续播放默认关闭，按当前筛选列表前进且不循环。

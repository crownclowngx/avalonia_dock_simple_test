...
/// <summary>验证 MyPlugTest 私有消息器的同步派发、重入、并发和释放契约。</summary>
public sealed class MyPlugTestEventBusTests
{
    [Fact]
    public void 按订阅顺序在发布线程同步派发且只匹配精确类型()
    {
        using var eventBus = new MyPlugTestEventBus();
    ...
    {
        using var firstBus = new MyPlugTestEventBus();
        using var secondBus = new MyPlugTestEventBus();
        var firstCount = 0;
        var secondCount = 0;
    ...
    {
        var eventBus = new MyPlugTestEventBus();

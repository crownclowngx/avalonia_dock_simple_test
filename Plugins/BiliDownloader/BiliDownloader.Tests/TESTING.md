...
- P1-G4 Document V3 组：V3 往返、非当前格式拒绝、离线恢复、缺失 Provider、可复用预设和敏感字段快照。
...
P1-G6 自动化基线（2026-08-07）：BiliDownloader Release 完整门禁 628/628 通过、0 跳过；覆盖率为总体行 84.46% / 分支 67.56%，A 组 90.75% / 78.54%，B 组 84.60% / 67.70%，C 组 76.03% / 57.72%，全部超过现行门禁。全解决方案 Release 回归 1024/1024 通过、0 跳过。

P1-G7 自动化基线（2026-08-07）：BiliDownloader Release 完整门禁 660/660 通过、0 跳过；覆盖率为总体行 84.34% / 分支 67.76%，A 组 90.27% / 77.98%，B 组 84.40% / 68.20%，C 组 75.96% / 57.40%，全部超过现行门禁。固定 ffmpeg/ffprobe 8.1.2 离线媒体验收对 4 个成品验证通过，0 转码参数；全解决方案 Release 回归 1056/1056 通过、0 跳过。
...
- 历史测试使用临时 SQLite 和文件探测替身；Tool 激活与查询必须断言零磁盘探测，Inaccessible 分支不得依赖开发机权限配置。
- 导出测试使用 1000 条本地任务验证流式路径，并扫描最终字节中不存在 Cookie、Authorization、签名参数、Cover URL、临时目录和堆栈。

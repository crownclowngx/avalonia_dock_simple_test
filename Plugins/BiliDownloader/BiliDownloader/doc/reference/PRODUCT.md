...
> 2026-08-22 Host V3 G12 验收更新：在不改变下载、认证、SQLite、FFmpeg、限速、任务恢复及
> content schema 3 的前提下，真实 Host 保存链、插件私有消息、Lifecycle/readiness、1 Document +
> 1 Tool + 1 Lifecycle 的 Workspace 组合和两次确定性 win-x64 ZIP 已通过。共 **1219/1219**，
> 插件覆盖率 **83.80% / 67.54%**。本轮不使用 AIFLOW，不运行 Windows CI/Smoke、
> ReleaseAcceptance 或发布门禁，测试 ZIP 不可发布。详细证据见
> [G12 BiliDownloader Host V3 验收](../../../../../docs/plan-history/host-v3/g12-bili-downloader-v3-acceptance.md)。
...
> 历史 P0/P1 发布与验收记录按当时事实保留；它们不是 Host V3 G12 的执行证据，也不会因为本次验收
> 被改写为已经发布。

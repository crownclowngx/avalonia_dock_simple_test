...
| `ciphertext` | Cookie、缓存用户名和头像序列化后的 AES-GCM 密文 |
...
3. `BiliCredentialStore` 序列化 Cookie、用户名和头像组成的完整会话载荷。
...
1. 应用启动时先读取本地密文并立即恢复内存登录态和缓存账号资料。
2. 插件生命周期随后启动最长 5 秒的非阻塞后台验证，不等待网络即可继续显示主窗口。
3. 验证成功时刷新真实用户名和头像，并将 v1 Cookie 载荷自动升级为 v2 完整会话载荷。
4. 网络不可用或超时时保留密文与缓存资料，不把“无法验证”误判为“凭据无效”。
5. Bilibili 明确判定无效时删除持久化凭据并切换为未登录。
6. 密文、tag、key 或载荷损坏时删除不可读状态，重新登录后生成新 key。
...
`BiliDownloader.Tests` 当前 21 项测试全部通过，其中 G1 新增或扩展的验证包括：
...
- 用户名和头像与 Cookie 一起加密，数据库中不存在这些资料的明文；
- v1 Cookie 密文可以读取，并在后台验证成功后自动升级为 v2；
- 新进程立即从本地密文恢复登录状态，无需再次点击登录按钮；
- 后台验证成功、明确失效、网络不可用、关闭取消和旧结果竞态均有覆盖；
- 新任务表不存在 `cookie` 列；

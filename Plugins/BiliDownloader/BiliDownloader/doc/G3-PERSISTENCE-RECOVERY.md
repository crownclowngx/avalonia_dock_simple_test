...
- FlushAsync 超时 2 秒后放弃。极端场景（SQLite 锁争用）下可能丢失最后一份节流进度，但终态写入不受影响。
- 错误分类基于异常类型和消息字符串匹配，未引入异常层次结构。G7 错误行动入口时可细化。
- 临时文件校验只读大小，不验证内容完整性。P2 专业归档阶段的完整性检查可补充哈希校验。
- `DownloadProgressTracker` 中 `_lastProgressDbWrite`/`_lastBytesDbWrite` 字典在任务删除后不清理。长期运行可能有微量内存泄漏，G4 可补充清理逻辑。

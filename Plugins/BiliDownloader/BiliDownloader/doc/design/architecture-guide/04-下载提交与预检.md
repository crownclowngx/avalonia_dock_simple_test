...
`DownloadProfileSnapshot.PerTaskRateLimitBytesPerSecond` 是执行策略快照，不属于输出版本身份。提交模型统一拒绝负数、1～63 KiB/s 和换算溢出；0 表示不限速。Coordinator 在锁内创建任务时写入 `SubmissionSnapshotVersion=4` 与 `TaskRateLimitBytesPerSecond`。预检不把限速用于空间、冲突或媒体规格计算，因为它只改变完成时间，不改变输出字节与路径。

历史 v0～v3 没有可证明的限速事实，重新下载时回退 0。不能从旧速度文本反推限制值：`SpeedText` 是瞬时观测，限速字段才是用户意图。

...
`BiliDownloaderViewModel` 是普通 `ObservableObject`，实现最终
`IPersistablePluginDocument`。保存逻辑委托给 `IBiliDownloaderDocumentStateMapper` 和
`DocumentSaveCodec`；Host 独占文件路径、六字段信封、原子事务和保存成功提交点。
...
flowchart TD
    ...
    Version -->|非 V3| Reject["拒绝当前内容"]
    Validate --> Offline["离线恢复 UI，不主动访问 Provider"]
```

恢复来源时不会自动调用 `Normalize`、`GetPage` 或 `Resolve`。Document 打开只是恢复意图，不应因加载文件产生网络副作用。

当前只接受内容 schema 3。非 V3、正文损坏或安全校验失败均拒绝，不猜测恢复旧字段；宿主仅在最近
成功恢复备份可在新 Scope 中完整初始化时提供恢复副本入口。插件不读取 V1/V2 Host 信封。

schema 3 使用原生 `JsonElement` 和 `System.Text.Json`，保持既有 PascalCase 字段、数值枚举与缺失可选
字段默认值。未知字段可以读取，但不会进入领域状态，再次保存时自然丢弃。恢复时 Codec 与 Mapper 先
完整解码、规范化和验证候选状态，成功后一次应用；失败不发布半成品 Scope。`CaptureContentAsync`
只捕获内容，不清理脏状态，只有 Host 主文件成功写入后调用 `AcceptChanges` 才提交。

新建、`quick-url`、`personal-source` 与恢复统一从
`InitializeAsync(DocumentActivation, token)` 进入，并在 View 发布前完成。默认和两个 Intent 只由
`NewDocumentActivation` 表达，恢复只由 `RestoreDocumentActivation` 表达。视觉树不再承担
初始化；强制注入的 `IDocumentLifetime.ClosingToken` 会与调用令牌链接，贯穿解析、恢复、预检和子
ViewModel，关闭后的迟到结果不能写回 UI。
...
- settings 使用 `global_media_rate_limit_bytes_per_second`，生命周期初始化时加载；损坏值本次回退 0，但不覆盖原文。
- 任务表使用 `task_rate_limit_bytes_per_second INTEGER NOT NULL DEFAULT 0`；旧库幂等加列。
- 新提交快照版本为 4；旧任务缺列或旧版本均解释为不限速。
- 活动任务编辑先执行 repository 更新，再热更新任务 limiter 和 UI 投影；SQLite 失败时三者保持旧值。
- 完成任务禁止修改限速快照，避免历史事实随当前设置漂移。

限速变化通过 Coordinator 事件刷新 UI，但 SQLite 仍是跨进程事实源。重启恢复时 limiter 读取任务快照重新激活，不依赖上次会话的内存对象或速度文本。

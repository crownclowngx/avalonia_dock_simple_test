...
当前仓库已完成 V2 G14 封板。Host 使用 `DocumentActivationContext` 异步初始化普通 `IPluginDocument`；
...
`IDocumentLifetime`。SDK 版本为 2.0.0，Core 的 85 条 public 签名已进入 v2 Shipped。
四个业务插件均已迁移并通过确定性 ZIP 与真实 Host 加载；仓库不会自动推送公共包源。

...
当前仓库已完成未发布 V3 G13。Host 生产模块入口和声明式贡献目录使用本程序集，注册时一次绑定 Descriptor、
...
插件不取得 Window、Dock、owner 或 `TryRestore`。当前未发布 `3.0.0` UI SDK 的 45 条 public 签名位于
v3 Unshipped，v3 Shipped 为空。V3 G9–G12 只用既有声明和 Host internal Workspace/Dock Adapter
验证四插件，G13 又以真实 nupkg 负例证明旧 owner API 不可消费；这些阶段都没有增加公共类型、接口或成员。v2 Shipped

...
- V2 G14 已将 Core/UI public API 冻结到 v2 Shipped；V3 G1 的相同表面位于 v3 Unshipped，Host 自有实现不属于插件 API；
- `managed-plugin-v1.0.0` 只定位 Managed Plugin v1 历史基线，当前版本边界以 V3 G1、运行语义以 V2 G14 为准；
...
V3 G1 只运行上述非发布验证；Windows Smoke、ReleaseAcceptance 和发布总门禁留到 V3 G14。

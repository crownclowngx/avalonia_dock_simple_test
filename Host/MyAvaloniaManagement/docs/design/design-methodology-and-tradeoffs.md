...
[V20 归档方案](../../../../docs/archive/plans/host-v20-layout-retirement-plan.md)落实同一判断标准：产品不再需要的 Layout V2 运行能力退出，有效行为测试改用当前 V3，旧分组查询入口删除。Session、Store 和保存队列仍各自保持所有权；没有新增版本框架或旧文件清理器。目录测试直接观察分类树与完整创建身份。Workflow 取消锁边界与 Manifest 最小整理仍是独立候选，不混入布局退役。
...
当前只支持严格 manifest、显式贡献和独立插件 Provider。Document 采用独立 Scope、互斥激活与修订保存；Layout 仅生产读写 schema 3，不探测或导入旧布局；当前合并与偏好校验只排除明确退役的内置 Tool。Document V1、Layout V1 与旧策略发现不回流到生产路径。
...
SOLID 首先落实在测试基础设施：构建目标选择 RestartHarness 运行文件，复制助手只准备独立可写副本并计量，真实进程场景继续断言生产协议。Windows x64 副本保留当前平台及通用 win 运行资产、托管 PDB 和依赖描述，排除原生调试符号、其他平台文件及证据；非 Windows 保留原运行资产，不扩大平台支持承诺。没有共享可写缓存或硬链接。

UI 的排队通知、渲染推进、条件完成分别由三个小方法表达；输入动作只发一次。恢复浮窗的可见状态不等于两个工具组已完成布局，关闭前必须确认被测内容就绪，不能靠固定睡眠或重试点击掩盖时序。帮助导航用显式进入/释放信号安排竞态。行为、布局、像素断言和辅助截图各自保留原责任。

Gate 使用固定顺序计划，保留普通委托作为阶段自测边界，不引入通用依赖图。单仓快照直接传递，scope 仅保留兼容标签。TRX reader 读取不可变字节快照并校验结构，Gate 决定完整成功，旁路摘要引用同一结果；失败和未执行不能伪装成零测试通过。发布顺序、资格和阈值不随开发契约前置而变化。实现、测量与适用限制见[V21 开发记录](../../../../docs/archive/records/host-v21/development-acceptance.md)。

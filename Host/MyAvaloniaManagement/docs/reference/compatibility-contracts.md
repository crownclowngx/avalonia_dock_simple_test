...
> 用途：内部重构必须保护的外部可观察行为。状态：当前；核对日期：2026-09-21。事实源：当前 SDK public 类型、Host Loader/Registry/Workspace、文档与布局实现及对应测试。
...
V18 仅改变命令面板的分组展示、默认选择和提交约束；强类型身份、声明发现许可、原创建落点、菜单/快捷键和取消排空保持。页面命令携带本次展示的 PageId/ContextRevision，旧上下文提交被拒绝，不主动切页后执行；全局入口不携带页面约束。SDK 和持久化格式不变，插件原因仅有布尔状态时使用通用说明。验证见[V18 专用开发验证](../../../../docs/maintenance/host-v18-command-palette-verification.md)。

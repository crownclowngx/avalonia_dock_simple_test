...
/// <summary>验证 Command 关闭门控只有在排空后才允许继续释放工作区所有权。</summary>
public sealed class WorkbenchCommandShutdownGateTests
{
    [Fact]
    public void 排空成功允许继续释放且先关闭入口()
    {
        var participant = new FakeParticipant(Task.FromResult(true));
        var gate = new WorkbenchCommandShutdownGate(participant);
    ...
    [Fact]
    public void 宽限超时记录脱敏诊断并禁止释放所有权图()
    {
        var participant = new FakeParticipant(Task.FromResult(false));
        var diagnostics = new RecordingDiagnosticSink();
        var gate = new WorkbenchCommandShutdownGate(participant, diagnostics);
        ...
        Assert.Equal(HostDiagnosticCodes.WorkbenchCommandShutdownTimeout, draft.Code);

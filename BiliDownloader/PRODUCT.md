...
| 登录凭据 | 凭据存储 | 与任务库隔离，目标为 Windows DPAPI 加密 |
...
| 架构 | 插件初始化与有序关闭 | 已实现 | 仅 BiliDownloader 选择接入；其他插件继续使用历史流程 |
| 依赖 | ffmpeg 检测与自定义路径 | 已实现 | 尚无一键安装修复 |
| 安全 | Cookie 与任务解耦 | 部分实现 | 模型已标记废弃，但仍有过渡写入 |
| 安全 | DPAPI 加密 Cookie | 未实现 | P0 |
...
| P0-08 | ffmpeg 一键就绪 | 一键安装精简版本、校验、重新选择和修复 | Tool / ffmpeg 服务 |
| P0-09 | 凭据安全 | DPAPI 加密；任务表不保存 Cookie；日志不记录敏感值 | 凭据服务 / SQLite |
| P0-10 | 错误行动入口 | 登录、ffmpeg、目录、磁盘、网络错误提供对应修复按钮 | Tool / Document |
...
| P1-03 | 可复用方案 | 完整保存内容源、筛选、预设、命名和输出规则 | Document 保存机制 |
| P1-04 | 增量检查 | 区分新增、已下载、下载中、失效和规则排除内容 | Document / SQLite |
| P1-05 | 输出控制 | AVC/HEVC/AV1，MP4/MKV，仅视频/仅音频 | Document / 下载引擎 |
| P1-06 | 高规格媒体 | 标识并选择 HDR、杜比视界、Hi-Res 和杜比全景声 | Document / Bili API |
| P1-07 | 字幕弹幕增强 | 字幕语言选择、多格式、软字幕内嵌；弹幕 XML/ASS/JSON | Document / Extras |
| P1-08 | 历史中心 | 搜索、重新下载、定位文件、存在性检查、CSV/JSON 导出 | Tool / SQLite |
| P1-09 | 速度控制 | 全局和单任务限速，保留并发数设置 | Tool / 下载引擎 |

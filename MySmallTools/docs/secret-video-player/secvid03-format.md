...
- SECVID03 解析器要求魔数、版本、固定常量和物理长度全部匹配，不做宽松猜测。
- SECVID02 属于旧的完整内存解密格式，当前播放器只识别其魔数并显示“请重新加密”，不会尝试播放。
- SECVID02 文件应保留原始视频或使用旧工具恢复后，再通过当前“视频文件加密器”生成 SECVID03。
- 不得通过修改 SECVID03 常量来“兼容”新需求。需要改变磁盘布局、KDF、块大小或认证规则时，应设计新版本魔数，并保留显式迁移路径和测试向量。
